...
dens <- 100 # Setting density
p_grid <- seq( from = 0 , to = 1 , length.out = dens ) # Defining grid
prior <- rep( 1 , dens ) # Defining prior
likelihood <- dbinom( 6 , size = 9 , prob = p_grid ) # Finding likelihood for each value in grid
unstd.posterior <- likelihood * prior # Finding product of likelihood and prior
posterior <- unstd.posterior / sum(unstd.posterior) # Standardizing posterior to sum to 1
